...
void main() {
	...
	vec3 L = normalize(light_pos - vertex_position_worldspace);
	vec3 H = normalize(L + V);

	float distance = length(light_pos - vertex_position_worldspace);
	float attenuation = 1.0 / (distance * distance);

	vec3 radiance = light_color_bigger * attenuation;
